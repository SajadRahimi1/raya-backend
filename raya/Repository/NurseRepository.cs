...
public class NurseRepository : INurseRepository
{
    ...
    public async Task<List<Nurse>> GetAllNurse(int page)
    {
        ...
        // {
        page = page < 1 ? 1 : page;
        nurses = await _appDbContext.Nurses.OrderByDescending(nurse => nurse.UpdatedAt).Where(nurse => nurse.authority != null && nurse.status == null).Skip((page - 1) * 15).Take(15).ToListAsync();
    ...
    {
        var nurse = await _appDbContext.Nurses.Include(_ => _.NurseImages).Include(_ => _.NurseFamily).Include(_ => _.NurseCategories).Include(_ => _.Shift).AsNoTracking().SingleOrDefaultAsync(_ => _.Id.ToString() == id);
        if (nurse == null)
        {
